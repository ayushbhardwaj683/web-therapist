      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-[90rem] mx-auto">
        ...
        {specialties.map((item, index) => (
          ...
          <ScrollAnimator
            ...
            delay={index * 100}
            // GRID SPAN LOGIC:
            // On tablet (md), the 3rd item (index 2) spans 2 columns to center it.
            // On desktop (lg), it goes back to spanning 1 column.
            className={`h-full flex ${index === 2 ? 'md:col-span-2 lg:col-span-1 md:w-1/2 md:mx-auto lg:w-full' : ''}`}
          >
            <div className="flex flex-col h-full w-full p-8 border border-[#3B3632]/20 bg-[#dcd8d0] hover:bg-[#E8E4DE] transition-colors duration-300">
              
              <div className="flex-grow">
                <h3 className="text-2xl md:text-3xl font-serif font-bold mb-6 text-[#3B3632]">
                  {item.title}
                </h3>
              ...
              {/* IMAGE CONTAINER FIX:
                  - Removed fixed width/height (w-64 etc).
                  - Used max-w and aspect-ratio to keep it circular and responsive.
              */}
              <div className="w-full flex justify-center mt-auto pt-8">
                <div className="w-full max-w-[280px] aspect-square relative rounded-full overflow-hidden border-2 border-[#E8E4DE] shadow-sm flex-shrink-0">
                  <img
                    src={item.image}
                    alt={item.alt}
                    className="w-full h-full object-cover"
